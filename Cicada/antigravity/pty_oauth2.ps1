...
def capture():
    cmd = ["/root/.nvm/versions/node/v22.22.1/bin/node", "/root/.nvm/versions/node/v22.22.1/lib/node_modules/openclaw/openclaw.mjs", "models", "auth", "login-github-copilot"]
    env = os.environ.copy()
    ...
    if pid == 0:
            ...
            try:
                ...
                if fd in r:
                    ...
                    if not data:
                        break
                    out += data
                    if b"login/device" in out or b"code" in out:
                        time.sleep(1)
                        data2 = os.read(fd, 1024)
                        out += data2
                        break
            except OSError:
                break
        ...
        # We don't kill the child, because we want it to keep waiting for the user to auth!
        # Just detach and print the message.
        import signal
...
Run "Run PTY wrapper" "python3 /tmp/pty.py 2>/dev/null &"
Run "Check PTY output" "sleep 3; cat /tmp/oauth.txt"
